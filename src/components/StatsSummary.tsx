...
export function summarizeLogs(logs: Array<{ rating: number; movie?: { title?: string; genre?: string[]; language?: string | null; country?: string | null } | Array<{ title?: string; genre?: string[]; language?: string | null; country?: string | null }> }>) {
  ...
  return { total: logs.length, genre: top(movies.flatMap((movie) => movie.genre ?? [])), language: top(movies.map((movie) => movie.language).filter(Boolean) as string[]), country: top(movies.map((movie) => movie.country).filter(Boolean) as string[]) };
}
