...
import { Clapperboard, Film, LogOut, Plus, BarChart3, Settings } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const nav = [{ href: "/movies", label: "My Films", icon: Film }, { href: "/stats", label: "The numbers", icon: BarChart3 }, { href: "/settings", label: "Settings", icon: Settings }];
  ...
  return <div className="min-h-screen bg-[var(--ink)] text-[var(--paper)]">
    <header className="site-header"><Link href="/movies" className="brand"><Clapperboard size={22} /><span>Cinefolio</span></Link>
      <nav className="flex items-center gap-1 sm:gap-2">{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={cn("nav-link", pathname.startsWith(href) && "nav-link-active")}><Icon size={16} /><span className="hidden sm:inline">{label}</span></Link>)}</nav>
