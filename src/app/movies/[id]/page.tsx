...
export default async function MovieDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from("logs").select("*, movie:movies(*)").eq("id", id).single();
  if (!data) notFound();
  ...
  return <AppShell>
    ...
    <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
      <div className="poster-frame h-fit lg:sticky lg:top-24">{log.movie.poster_url ? <img src={log.movie.poster_url} alt={`${log.movie.title} poster`} /> : <div className="poster-placeholder">No poster yet</div>}</div>
      <div>
        ...
        <div className="mt-8 border-t border-[var(--line)] pt-5"><DeleteLogButton id={log.id} /></div>
      </div>
    </div>
  </AppShell>;
}
...
function Meta({ label, value }: { label: string; value: string | number | null | undefined }) {
  return <div><p className="text-xs uppercase tracking-[.12em] text-[var(--muted)]">{label}</p><p className="mt-1 text-[var(--paper)]">{value || "Not recorded"}</p></div>;
}
